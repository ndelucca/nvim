...
nmap("<leader>f/", function()
	builtin.history({ scope = "/" })
end, '"/" history')
nmap("<leader>f:", function()
	builtin.history({ scope = ":" })
end, '":" history')
nmap("<leader>fa", function()
	builtin.git_hunks({ scope = "staged" })
end, "Added hunks (all)")
nmap("<leader>fA", function()
	builtin.git_hunks({ path = "%", scope = "staged" })
end, "Added hunks (buf)")
nmap("<leader>fd", function()
	builtin.diagnostic({ scope = "current" })
end, "Diagnostic buffer")
nmap("<leader>fg", function()
	builtin.grep_live({ show_hidden = true })
end, "Live grep")
nmap("<leader>fG", function()
	builtin.grep({ show_hidden = true, pattern = vim.fn.expand("<cword>") })
end, "Grep word")
nmap("<leader>fh", builtin.help, "Help tags")
nmap("<leader>fb", buffer_ui.toggle_quick_menu, "Buffer manager")
